...
# Goals:
# - Generate Planets from array of (location, mass, size)
# - Save picture from pygame
# - Find planet colors with KMeans -- explain later too
# - Generate 10,000 samples

# - Train the CNN with outcomes
...
class Planet:
    def __init__(self, x, y, radius, mass, id, color):
        self.x = x
        self.y = y
        self.radius = radius
        self.mass = mass
...
# planets.append(Planet(836, 472, 500, 300, planet_id, (255, 0, 0)))
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONDOWN and event.button == 1:
            mouse_down = True
            if not ctrl_down:
                planets.append(Planet(mx, my, 0, 0, planet_id, (255, 0, 0)))
                print(mx, my)
                planet_id += 1
